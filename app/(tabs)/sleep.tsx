...
export default function SleepScreen() {
  ...
  const buildWeekly = useCallback((all: SleepLog[]) => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      data.push({ date: key, value: log?.durationMin ?? 0 });
    }
    setWeeklyData(data);
  }, []);
        ...
        .upsert(
          {
            ...
            quality,
            sleep_start: bedtime.toISOString(),
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      ...
      >
            ...
            {sleepHrs ? (
              <>
                <Text style={[styles.heroValue, { color: theme.text }]}>
                  {sleepHrs}
                  <Text style={[styles.heroUnit, { color: theme.textSecondary }]}> hrs</Text>
                </Text>
                ...
                <Text style={[styles.sleepRange, { color: theme.textSecondary }]}>
                  {lastNight?.sleepStart ? format(new Date(lastNight.sleepStart), 'h:mm a') : '—'} →{' '}
                  {lastNight?.sleepEnd ? format(new Date(lastNight.sleepEnd), 'h:mm a') : '—'}
                </Text>
              </>
            ) : (
              <Text style={[styles.emptyHero, { color: theme.textSecondary }]}>
                No sleep logged yet. Tap "Log Sleep" to add last night's sleep.
              </Text>
            )}
          </LinearGradient>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(120).springify()} style={styles.statsRow}>
          <View style={[styles.statCard, { backgroundColor: theme.card }]}>
            <Text style={[styles.statValue, { color: theme.text }]}>
              {avgDuration > 0 ? (avgDuration / 60).toFixed(1) : '—'}
            </Text>
        ...
        {/* Weekly Chart — tap a night to view details */}
        {weeklyData.length > 0 && (
          <Animated.View entering={FadeInDown.delay(180).springify()}>
            <View style={[styles.card, { backgroundColor: theme.card }]}>
              ...
              <DayBars
                ...
                formatValue={(v) => (v > 0 ? `${(v / 60).toFixed(1)}h` : '')}
            ...
            {selectedLog ? (
              <View>
                <Text style={[styles.detailDuration, { color: theme.text }]}>
                  {(selectedLog.durationMin / 60).toFixed(1)}
                  <Text style={[styles.detailUnit, { color: theme.textSecondary }]}> hrs</Text>
                </Text>
                <View style={[styles.starsRow, { marginTop: 6 }]}>
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Ionicons
                      key={s}
                      name={s <= (selectedLog.quality ?? 0) ? 'star' : 'star-outline'}
                      size={16}
                      color={s <= (selectedLog.quality ?? 0) ? theme.sleep : theme.border}
                    />
                  ))}
                </View>
                <Text style={[styles.detailRange, { color: theme.textSecondary }]}>
                  {selectedLog.sleepStart
                    ? format(new Date(selectedLog.sleepStart), 'h:mm a')
                    : '—'}{' '}
                  → {selectedLog.sleepEnd ? format(new Date(selectedLog.sleepEnd), 'h:mm a') : '—'}
                </Text>
              </View>
            ) : (
            ...
            logs.slice(0, 7).map((log) => (
              <TouchableOpacity
                ...
                style={[
                  styles.logRow,
                  {
                    ...
                    borderColor: log.date === selectedDate ? theme.sleep : 'transparent',
                    borderWidth: 1.5,
                  },
                ]}
                activeOpacity={0.8}
              ...
              >
                ...
                <View style={styles.logInfo}>
                  ...
                  <Text style={[styles.logDur, { color: theme.textSecondary }]}>
                    {(log.durationMin / 60).toFixed(1)} hrs
                  </Text>
                </View>
                {log.quality ? (
                  <View style={styles.miniStars}>
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Ionicons
                        key={s}
                        name={s <= log.quality! ? 'star' : 'star-outline'}
                        size={11}
                        color={s <= log.quality! ? theme.sleep : theme.border}
                      />
                    ))}
                  </View>
                ) : null}
...
const styles = StyleSheet.create({
  ...
  detailDuration: { fontSize: 36, fontWeight: '800', letterSpacing: -1 },
  detailUnit: { fontSize: 16, fontWeight: '500' },
  detailRange: { fontSize: 14, fontWeight: '500', marginTop: 10 },
