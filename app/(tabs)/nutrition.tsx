  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
        <Text style={{ fontSize: 11, fontWeight: '700', color: '#9CA3AF', textTransform: 'uppercase' }}>{label}</Text>
      ...
      <View style={{ height: 6, borderRadius: 3, backgroundColor: color + '22', overflow: 'hidden' }}>
        <View style={{ width: `${Math.round(pct * 100)}%` as `${number}%`, height: '100%', backgroundColor: color, borderRadius: 3 }} />
      </View>
    </View>
  );
}

function MealCard({ meal, onDelete, theme }: { meal: Meal; onDelete: (id: string) => void; theme: ReturnType<typeof import('@/hooks/useTheme').useTheme> }) {
  ...
  return (
    <View style={[mealCardStyles.row, { backgroundColor: theme.card }]}>
      ...
      <TouchableOpacity onPress={() => onDelete(meal.id)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
...
const mealCardStyles = StyleSheet.create({
  row: { flexDirection: 'row', alignItems: 'center', padding: 14, borderRadius: 14, marginBottom: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 1 },
...
export default function NutritionScreen() {
  ...
  const fetchMeals = useCallback(async () => {
    ...
    if (data) {
      dispatch(setMeals(data.map((r) => ({
        id: r.id, name: r.name, mealType: r.meal_type,
        calories: r.calories ?? undefined, proteinG: r.protein_g ?? undefined,
        carbsG: r.carbs_g ?? undefined, fatG: r.fat_g ?? undefined,
  ...
  const handleEstimate = useCallback(async () => {
    if (!mealName.trim()) { showToast('Enter a meal name first', 'error'); return; }
    setEstimating(true);
    try {
      ...
      if (match) {
        const m = JSON.parse(match[0]) as { calories?: number; proteinG?: number; carbsG?: number; fatG?: number };
  ...
  const handleAddMeal = useCallback(async () => {
    if (!user?.id || !mealName.trim()) { showToast('Meal name is required', 'error'); return; }
    setSaving(true);
    const tempMeal: Meal = {
      id: `tmp-${Date.now()}`, name: mealName.trim(), mealType,
    ...
    try {
      const { data, error } = await supabase
        .from('meals')
        .insert({
          user_id: user.id, name: tempMeal.name, meal_type: mealType,
          calories: tempMeal.calories ?? null, protein_g: tempMeal.proteinG ?? null,
          carbs_g: tempMeal.carbsG ?? null, fat_g: tempMeal.fatG ?? null,
          logged_at: tempMeal.loggedAt,
        })
        .select().single();
      if (error) throw error;
      dispatch(removeMeal(tempMeal.id));
      dispatch(addMeal({ id: data.id, name: data.name, mealType: data.meal_type, calories: data.calories ?? undefined, proteinG: data.protein_g ?? undefined, carbsG: data.carbs_g ?? undefined, fatG: data.fat_g ?? undefined, loggedAt: data.logged_at }));
  ...
  const mealsByType = useMemo(() => {
    ...
    todayMeals.forEach((m) => { groups[m.mealType].push(m); });
    return groups;
  }, [todayMeals]);
          ...
          >
            <View style={styles.calorieRow}>
              <View>
                ...
                <Text style={[styles.calorieLabel, { color: theme.textSecondary }]}>kcal today</Text>
              </View>
              <Text style={{ fontSize: 36 }}>🥗</Text>
            </View>
            {(totalProt > 0 || totalCarb > 0 || totalFat > 0) && (
              <View style={styles.macroRow}>
                <MacroBar label="Protein" value={Math.round(totalProt)} max={150} color={theme.tint} />
                <MacroBar label="Carbs" value={Math.round(totalCarb)} max={300} color={theme.nutrition} />
        ...
        <BottomSheetScrollView contentContainerStyle={[styles.sheetContent, { backgroundColor: theme.card }]}>
          ...
          <View style={styles.typeRow}>
            {MEAL_TYPES.map((mt) => (
              <TouchableOpacity
                key={mt.key}
                style={[styles.typeChip, { backgroundColor: mealType === mt.key ? theme.nutrition : theme.background, borderColor: mealType === mt.key ? theme.nutrition : theme.border }]}
                onPress={() => setMealType(mt.key)}
              >
                <Text style={{ fontSize: 16 }}>{mt.emoji}</Text>
                <Text style={[styles.typeChipText, { color: mealType === mt.key ? '#fff' : theme.text }]}>{mt.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {/* AI Estimate */}
          <TouchableOpacity
            style={[styles.aiBtn, { backgroundColor: theme.tint + '18', borderColor: theme.tint + '40' }]}
          ...
          <TouchableOpacity
            style={[styles.saveBtn, { backgroundColor: theme.nutrition, opacity: saving ? 0.7 : 1 }]}
...
const styles = StyleSheet.create({
  ...
  summaryCard: { borderRadius: 22, padding: 20, marginBottom: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 8, elevation: 3 },
  ...
  mealTypeHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8, marginTop: 4 },
  ...
  typeChip: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 20, borderWidth: 1.5 },
  typeChipText: { fontSize: 13, fontWeight: '700' },
  aiBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 13, borderRadius: 14, borderWidth: 1 },
